...
for s=1:S    
    ...
    u_res(:,:,s)=rand(T,N);
    [~,~,~,lik_indv(:,s)]=bootstrap_LGSS_corr_DE(yt,D,Z,sigma_u,C,G,M,sigma_e,u_particles_init(:,:,s),u_particles(:,:,:,s),u_res(:,:,s),N);
end
toc
post = sum(lik_indv) + prior_psi;
...
for i=1:nloop
    ...
    u_particles_init_star = corr_param*u_particles_init+sqrt(1-corr_param^2)*randn(dim_y,N);    
    u_particles_star=corr_param*u_particles+sqrt(1-corr_param^2)*randn(dim_y,N,T);
    u_res_norm=norminv(u_res);
    u_res_norm_star=corr_param*u_res_norm+sqrt(1-corr_param^2)*randn(T,N);
    u_res_star=normcdf(u_res_norm_star);
    for s=1:S
        [~,~,~,lik_indv_star(:,s)]=bootstrap_LGSS_corr_DE(yt,D,Z,sigma_u,C,G_star,M,sigma_e,u_particles_init_star(:,:,s),u_particles_star(:,:,:,s),u_res_star(:,:,s),N);
    end
    
    
    post_star = sum(lik_indv_star)+prior_psi_star;
...
%     end
    ...
    if mod(i,1000)==0
       save(['CorrPMMH_DE','dim',num2str(dim_y),'T',num2str(T),'N',num2str(N)],'Post'); 
    end
    toc
    scale=update_sigma(scale,C1,target_accept,i,1);
end
cputime=toc;
save(['CorrPMMH_DE','dim',num2str(dim_y),'T',num2str(T),'N',num2str(N)],'Post','cputime'); 
