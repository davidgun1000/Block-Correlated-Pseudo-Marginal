...
%indx=bsxfun(@times,ones(T,N),1:N); % preallocate
%particles(:,:,1)=bsxfun(@plus,xinit,chol(Vinit)*un); % particle initialisation
...
liki(i) = -0.5*l*log(2*pi) - 0.5*logdet(F) - 0.5*v'*(F\v);
