...
parfor s=1:S
    [~,~,~,lik_indv(:,s)]=bootstrap_LGSS(yt,D,Z,sigma_u,C,G,M,sigma_e,u_particles_init(:,:,s),u_particles(:,:,:,s),u_res(:,:,s),N);
end
...
for i=1:nloop
    ...
    for s=1:S
        u_particles_init_star(:,:,s) = randn(dim_y,N);
        u_particles_star(:,:,:,s)=randn(dim_y,N,T);
        u_res_star(:,:,s)=rand(T,N);
    end
    parfor s=1:S
        [~,~,~,lik_indv_star(:,s)]=bootstrap_LGSS(yt,D,Z,sigma_u,C,G_star,M,sigma_e,u_particles_init_star(:,:,s),u_particles_star(:,:,:,s),u_res_star(:,:,s),N);
    end
    ...
    Post.psi(i,1) = psi;
    if mod(i,1000)==0
       save(['ParPMMH_appr1_nocorr_',num2str(trim_first),num2str(trim_second),'dim',num2str(dim_y),'T',num2str(T),'N',num2str(N),'S',num2str(S),'.mat'],'Post');
 
    end
    
end
cputime=toc;
save(['ParPMMH_appr1_nocorr_',num2str(trim_first),num2str(trim_second),'dim',num2str(dim_y),'T',num2str(T),'N',num2str(N),'S',num2str(S),'.mat'],'Post','cputime');
