...
[lik_kalman_indv,~,~,~]=kalman(D,Z,sigma_u,M,sigma_e,G,yt');
lik_kalman = sum(lik_kalman_indv);
post = lik_kalman + prior_psi;
...
for i=1:nloop
    ...
    [lik_kalman_indv_star,~,~,~]=kalman(D,Z,sigma_u,M,sigma_e,G_star,yt');
    lik_kalman_star = sum(lik_kalman_indv_star);
    post_star = lik_kalman_star+prior_psi_star;
...
save(['Kalman',num2str(dim_y),'T',num2str(T),'.mat'],'Post');
