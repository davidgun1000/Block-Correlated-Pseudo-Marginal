...
lik_indv_appr1_trim = lik_indv_appr1_sort(1:S);
...
for i=1:nloop
    ...
    parfor s=1:S
        [particles_eps_star(:,:,:,s),particles_first_star(:,:,:,s),particles_second_star(:,:,:,s),w_star(:,:,s),indx_star(:,:,s),lik_indv_star(:,s)]=Mix_smc_SmallScale_2ndorder_bootstrap_corr(A_star,B_star,H_star,Phi_star,R_star,S2_star,Phi11_star,Phi12_star,R00_star,delta_ss_star,N,yt,u_particles_star(:,:,:,s),u_res_star(:,:,s),...
              state_var_positions,corr_index,disturbance_index);
    end
    ...
    lik_indv_appr1_trim_star = lik_indv_appr1_sort_star(1:S);
    ...
    if mod(i,250)==0
       save(['ParPMMH_appr1_','N',num2str(N),'S',num2str(S),'SmallScale_Datafull_bootstrap.mat'],'Post');
