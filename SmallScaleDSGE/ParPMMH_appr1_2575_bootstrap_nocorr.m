...
% end
parfor s=1:S
    [particles_eps(:,:,:,s),particles_first(:,:,:,s),particles_second(:,:,:,s),w(:,:,s),indx(:,:,s),lik_indv(:,s)]=Mix_smc_SmallScale_2ndorder_bootstrap_alt(A,B,H,Phi,R,S2,Phi11,Phi12,R00,delta_ss,N,yt,...
        state_var_positions);
end
...
for i=1:nloop
    ...
    parfor s=1:S
        [particles_eps_star(:,:,:,s),particles_first_star(:,:,:,s),particles_second_star(:,:,:,s),w_star(:,:,s),indx_star(:,:,s),lik_indv_star(:,s)]=Mix_smc_SmallScale_2ndorder_bootstrap_alt(A_star,B_star,H_star,Phi_star,R_star,S2_star,Phi11_star,Phi12_star,R00_star,delta_ss_star,N,yt,...
        state_var_positions);
    end
    ...
    if mod(i,250)==0
       save(['ParPMMH_appr1_nocorr_',num2str(trim_first),num2str(trim_second),'N',num2str(N),'S',num2str(S),'SmallScale_Data1_bootstrap.mat'],'Post');
