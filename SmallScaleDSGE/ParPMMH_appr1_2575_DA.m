...
jac_CDKF=log(1/param_tau)+log(1/param_psi1)+log(1/param_psi2)+...
...
for i=1:nloop
  ...
  prior_CDKF_star=log_truncatednormal(param_tau_star,prior_tau_mean,prior_tau_std,0,inf)+...
...
%       
    post_CDKF_star=lik_CDKF_star+prior_CDKF_star;
    
    


    jac_CDKF_star=log(1/param_tau_star)+log(1/param_psi1_star)+log(1/param_psi2_star)+...
    ...
    r1_CDKF = exp(post_CDKF_star-post_CDKF+jac_CDKF-jac_CDKF_star);
    C1 = min(1,r1_CDKF);
    rng('shuffle')
    A_rand = rand;
    
    if A_rand<=C1
        accept_CDKF=accept_CDKF+1;  
    ...
    if sum(isnan(lik_indv_star))>0 | sum(sum(sum(sum(isnan(particles_eps_star),4),3),2))>0 
       ...
       lik_indv=lik_indv;
       eps_ctraj=eps_ctraj;
       state_first_ctraj=state_first_ctraj;
       state_second_ctraj=state_second_ctraj;
       
    else
        
    for s=1:S
       [eps_ctraj_star(:,:,s),state_first_ctraj_star(:,:,s),state_second_ctraj_star(:,:,s)]=ancestor_trace_DSGE_2ndorder(particles_eps_star(:,:,:,s),particles_first_star(:,:,:,s),particles_second_star(:,:,:,s),w_star(:,:,s),indx_star(:,:,s));
    end
    ...
    r1_PF = exp(post_PF_star-post_PF-(post_CDKF_star-post_CDKF));
    ...
    if A_rand<=C1
        ...
        post_CDKF=post_CDKF_star;
        lik_indv=lik_indv_star;
        jac_CDKF=jac_CDKF_star;
        u_particles=u_particles_star;
        u_res=u_res_star;
        id_mix=id_mix_star;
        lik_PF=lik_PF_star;
        eps_ctraj=eps_ctraj_star;
        state_first_ctraj=state_first_ctraj_star;
        state_second_ctraj=state_second_ctraj_star;
    end
...
% = sum(params_normal.*(W*ones(1,n_params)));
%    aux = params_normal - ones(M,1)*est;
%    V1 = aux'*diag(W)*aux; 

    
    
    %Post.eps1(i,:)=mean(eps1_store);
    %Post.eps2(i,:)=mean(eps2_store);
    %Post.eps3(i,:)=mean(eps3_store);
    ...
    if mod(i,250)==0
       save(['ParPMMH_appr1_',num2str(trim_first),num2str(trim_second),'N',num2str(N),'S',num2str(S),'SmallScale_Data1_DA.mat'],'Post');
...
%     if mod(i,500)==0 & i>=burn+adapt
% 
%         param_piA_prop=mean(Post.piA(1:end,1)); 
%         param_tau_prop=mean(Post.tau(1:end,1));
%         param_psi1_prop=mean(Post.psi1(1:end,1));
%         param_psi2_prop=mean(Post.psi2(1:end,1));
%         param_gammaQ_prop=mean(Post.gammaQ(1:end,1));
%         param_rA_prop=mean(Post.rA(1:end,1));
%         param_BETA_prop=1/(1+param_rA_prop/400);
%         param_rhog_prop=mean(Post.rhog(1:end,1));
%         param_rhoz_prop=mean(Post.rhoz(1:end,1));
%         param_rhoR_prop=mean(Post.rhoR(1:end,1));
%         param_sigma_r_prop=mean(Post.sigma_r(1:end,1));
%         param_sigma_g_prop=mean(Post.sigma_g(1:end,1));
%         param_sigma_z_prop=mean(Post.sigma_z(1:end,1));
%         param_sigma_measure1_prop=mean(Post.sigma_measure1(1:end,1));
%         param_sigma_measure2_prop=mean(Post.sigma_measure2(1:end,1));
%         param_sigma_measure3_prop=mean(Post.sigma_measure3(1:end,1));
% 
% 
%        zeta=param_zeta;
%        nu=param_nu;
...
%        piA=param_piA_prop;
%        tau=param_tau_prop;
%        psi1=param_psi1_prop;
%        psi2=param_psi2_prop;
%        gammaQ=param_gammaQ_prop;
%        rA=param_rA_prop;
%        BETA=param_BETA_prop;
%        rhog=param_rhog_prop;
%        rhoz=param_rhoz_prop;
%        rhoR=param_rhoR_prop;
%        sigma_r=param_sigma_r_prop;
%        sigma_g=param_sigma_g_prop;
%        sigma_z=param_sigma_z_prop;
%        sigma_measure1=param_sigma_measure1_prop;
%        sigma_measure2=param_sigma_measure2_prop;
%        sigma_measure3=param_sigma_measure3_prop;
%        
%        save PARAMSTORE  BETA nu phi piA tau psi1 psi2 rhog rhoz rhoR sigma_z sigma_g sigma_r zeta gammaQ rA;
%        SmallScale_V3
%        f_11=[oo_.dr.ghx oo_.dr.ghu];
...
%        optPruning=[];
%        [omega_ss,F1,F2,omega_ss_delta,F11,F22,F12,~,~,~,~,~,state_var_positions,control_var_postion] = statespaceform(optPruning,oo_,M_,f_11,order);
%        A_prop(eq_y,1) = gammaQ;
%        A_prop(eq_pi,1) = piA;
%        A_prop(eq_ffr,1) = piA+rA+4*gammaQ; 
%        rng('shuffle')
%        %A_prop=A;
%        B_prop=B;
%        H_prop=diag([sigma_measure1^2;sigma_measure2^2;sigma_measure3^2]);;
%        Phi_prop=F1;
%        R_prop=F2;
%        S2_prop=M_.Sigma_e;
%        Phi11_prop=F11;
...
%        
%        parfor s=1:num_prop
%            [particles_eps_prop(:,:,:,s),particles_first_prop(:,:,:,s),particles_second_prop(:,:,:,s),w_prop(:,:,s),indx_prop(:,:,s),lik_prop(s,1)]=Mix_smc_SmallScale_2ndorder_PMMH_mix(A_prop,B_prop,H_prop,Phi_prop,R_prop,S2_prop,Phi11_prop,Phi12_prop,...
%                 R00_prop,delta_ss_prop,N_prop,yt,u_particles_prop(:,:,:,s),...
%                 u_res_prop(:,:,s),...
%                 w_mix,mean_mix,covmat_mix,...
%                 id_mix_prop(:,:,s),state_var_positions,0,0);
%            [eps_ctraj_prop(:,:,s),~,~]=ancestor_trace_DSGE_2ndorder(particles_eps_prop(:,:,:,s),particles_first_prop(:,:,:,s),particles_second_prop(:,:,:,s),w_prop(:,:,s),indx_prop(:,:,s));
%        end
%         
%        eps1_prop=(reshape(eps_ctraj_prop(1,:,:),T,num_prop))';
...
%     end
    ...
    %Post.state1_first(i,:)=mean(state1_first_ctraj_store);
    %Post.state2_first(i,:)=mean(state2_first_ctraj_store);
    %Post.state3_first(i,:)=mean(state3_first_ctraj_store);
    %Post.state4_first(i,:)=mean(state4_first_ctraj_store);
    %Post.state5_first(i,:)=mean(state5_first_ctraj_store);
    %Post.state6_first(i,:)=mean(state6_first_ctraj_store);
    %Post.state7_first(i,:)=mean(state7_first_ctraj_store);
    %Post.state8_first(i,:)=mean(state8_first_ctraj_store);
    
    
    %Post.state1_second(i,:)=mean(state1_second_ctraj_store);
    %Post.state2_second(i,:)=mean(state2_second_ctraj_store);
    %Post.state3_second(i,:)=mean(state3_second_ctraj_store);
    %Post.state4_second(i,:)=mean(state4_second_ctraj_store);
    %Post.state5_second(i,:)=mean(state5_second_ctraj_store);
    %Post.state6_second(i,:)=mean(state6_second_ctraj_store);
    %Post.state7_second(i,:)=mean(state7_second_ctraj_store);
    %Post.state8_second(i,:)=mean(state8_second_ctraj_store);