...
log(Rt/Rbar)=rhoR*log(Rt(-1)/Rbar)+(1-rhoR)*(gammapi*log(pit/pibar)+gammax*(xtg)+gammag*log(yt*exp(epsilonzt)/yt(-1))) +epsilonRt;
...
options_.noprint=1;
steady(solve_algo=1,maxit=500000,tolf=0.001);      
  

[eigenvalues_,result] = check(M_, options_, oo_);
